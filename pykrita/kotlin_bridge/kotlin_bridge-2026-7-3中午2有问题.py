...
class KotlinBridgeExtension(Extension):
    ...
    def get_available_devices(self):
        try:
            ...
            if result.returncode != 0:
                self.log(f"[Krita] adb devices 失败: {result.stderr.strip() if result.stderr else ''}")
                return []
    ...
    def choose_device(self):
        ...
        if not devices:
            QMessageBox.warning(
                None,
                "提示",
                "未找到可用的 adb 设备。请先连接手机并确保 adb devices 显示为 device。"
            )
            return None
    ...
    def start_bridge(self):
        ...
        self.device_id = self.choose_device()
        if not self.device_id:
            return

        self.server_thread = threading.Thread(target=self.run_server, daemon=True)
        self.server_thread.start()
        QMessageBox.information(None, "提示", f"已选择设备 {self.device_id}，服务正在启动。")
    ...
    def stop_bridge(self):
        ...
        if self.running_port != 0 and self.device_id:
            try:
                subprocess.run(
                    ["adb", "-s", self.device_id, "reverse", "--remove", f"tcp:{self.running_port}"],
    ...
    def run_server(self):
        try:
            ...
            self.log(f"[Krita] 选择设备: {self.device_id}")
            ...
            if result.returncode != 0:
                self.log(f"[Krita] adb reverse 失败: {result.returncode}")
                if result.stdout:
            ...
            try:
                ...
                self.log(f"[Krita] accept 失败，socket 已关闭: {e}")
                return
    ...
    def download_image_background(self, android_path):
        try:
            ...
            pull_cmd = ["adb", "-s", self.device_id, "pull", android_path, pc_image_path]
