...
class KotlinBridgeExtension(Extension):
    ...
    def send_client_message(self, msg):
        """
        线程安全地向当前已连接客户端发送一行文本（自动加换行并 flush）。
        如果没有客户端连接，会记录日志但不抛异常。
        """
        try:
            with self._client_lock:
                if self._client_writer:
                    try:
                        ...
                        try:
                            ...
                            pass
                        self.log(f"[Krita] 已发送给客户端: {msg}")
                    except Exception as e:
                        self.log(f"[Krita] 发送给客户端失败: {e}")
                else:
                    self.log("[Krita] send_client_message: 没有可用的 client_writer")
        except Exception as e:
