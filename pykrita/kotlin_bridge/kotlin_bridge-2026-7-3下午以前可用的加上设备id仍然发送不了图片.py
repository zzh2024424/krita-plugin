...
class KotlinBridgeExtension(Extension):
    ...
    def start_bridge(self):
        ...
        QMessageBox.information(None, "提示", f"已启动服务，正在拉起客户端...\n设备ID: {self.device_id}")
    ...
    def stop_bridge(self):
        ...
        if self.running_port != 0:
            try:
                subprocess.run(
                    self.adb_base_cmd() + ["reverse", "--remove", f"tcp:{self.running_port}"],
    ...
    def run_server(self):
        try:
            ...
            self.log(f"[Krita] 正在建立 adb 逆向映射端口: {self.running_port} / 设备: {self.device_id}")

            subprocess.run(
                self.adb_base_cmd() + ["reverse", f"tcp:{self.running_port}", f"tcp:{self.running_port}"],
                check=True,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
    ...
    def download_image_background(self, android_path):
        try:
            ...
            pull_cmd = self.adb_base_cmd() + ["pull", android_path, pc_image_path]
